...
router.get('/coininfo', function(req, res) {
  ...
  db.get_stats(settings.coin, function(stats){
    ...
    btcValue().then(value => {
    valorbtc=value
	var mcapusdcasero=(stats.supply * stats.last_price) * valorbtc;
        var preciousd=stats.last_price * valorbtc;
	    var blocksPerDay = (60*60*24)/settings.coininfo.block_time_sec;
            ...
            var monthlyUsd = formatCurrency(mnRewardsPerDay * preciousd * (365/12), { maxFraction: 2 });
            ...
            var yearlyUsd = formatCurrency(mnRewardsPerDay * preciousd * 365, { maxFraction: 2 });
	    var roiagno= (yearlyCoin / 2500) * 100;
            var data = {
              active: 'coininfo',
              coininfo: settings.coininfo,
              lastPriceBtc: formatCurrency(stats.last_price, { maxFraction: 8 }),
	      lastPrice: formatCurrency(stats.last_price, {maxFraction:8}),
              lastPriceUsd: cmc.price_usd ? formatCurrency(cmc.price_usd, { maxFraction: 6 }) : null,
              pricePercChange24h: cmc.percent_change_24h,
              marketCapUsd: formatCurrency(mcapusdcasero, { maxFraction: 2 }),
	      cmc: cmc,
	      roiagno: roiagno,
              blockCount24h: -1,
              avgBlockTime: -1,
              ...
              db.get_cmc(settings.coinmarketcap.ticker, function(cmc){
                db.get_stats(settings.coin, function (stats) {
                  ...
                  res.send({ data: [{
                    ...
                    supply: formatNum(stats.supply, { maxFraction: 4 }),
                    hashrate: hashrate,
	            lastPrice: formatNum(stats.last_price, { maxFraction: 8 }),
